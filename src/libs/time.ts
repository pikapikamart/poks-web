...
export const nextOccurrence = (
  ...
  if (content.recurrence === "none") return null;
...
export const dueTime = (
  ...
  if (content.dueAt) return content.dueAt;
  if (!content.dueDate || preferences.defaultDateHour === null) return null;
...
export const afterQuietHours = (iso: string, prefs: Preferences): string => {
  ...
  if (!quiet) return iso;
  if (start > end && h >= start) time = time.plus({ days: 1 });
  return time
